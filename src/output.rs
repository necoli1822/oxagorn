...
/// A detected tRNA / tmRNA gene — safe, owned (no raw pointers), for library
/// consumers. Coordinates are 1-based; `comp` is 0 (forward) / 1 (reverse).
#[derive(Clone, Debug)]
pub struct DetectedGene {
    /// Source sequence (contig) name.
    pub seqname: String,
    /// 1-based start (smaller coordinate for tRNA/tmRNA as emitted by detection).
    pub start: i64,
    /// 1-based stop.
    pub stop: i64,
    /// Strand: 0 = forward, 1 = reverse complement.
    pub comp: i32,
    /// Gene type: [`TRNA`] (0) or [`TMRNA`] (1).
    pub genetype: i32,
    /// Detection energy / score (higher = better).
    pub energy: f64,
    /// Number of introns (0 for most bacterial tRNAs).
    pub nintron: i32,
    /// Decoded anticodon triplet (uppercase DNA) for tRNA; empty for tmRNA.
    pub anticodon: String,
}

/// Decode a tRNA's anticodon triplet from its gene sequence (nucleotide codes
/// `0=A,1=C,2=G,3=T`), using the `anticodon` offset into `seq`.
fn decode_anticodon(g: &SendableGene) -> String {
    let a = g.anticodon;
    if a < 0 {
        return String::new();
    }
    ...
    for k in 0..3 {
        let idx = a as usize + k;
        if idx >= g.seq.len() {
            break;
        }
        s.push(match g.seq[idx] {
            0 => 'A',
            1 => 'C',
            2 => 'G',
            3 => 'T',
...
/// Detect tRNA / tmRNA genes in a FASTA file **in-process**, returning structured
/// results instead of writing formatted text.
///
/// This mirrors the detection half of [`parallel_fastafile`] (the parallel chunk
/// path) and reuses the very same [`process_chunk_parallel`] core + dedup, so the
/// gene calls are identical to the `oxagorn` binary for multi-contig / large
/// (`> MIN_CHUNK_PARALLEL_LEN`) inputs — the bactars use case. `sw` configures
/// detection (`trna`/`tmrna`/`both`/`geneticcode`/…); its output `FILE*` is not
/// touched. Intron-mode overlap removal (only active when `maxintronlen > 0`) is
/// not applied here. **Keep in sync with `parallel_fastafile`'s detection.**
pub unsafe fn detect_fastafile(
    filepath: &str,
    sw: *mut csw,
    num_threads: usize,
) -> Result<Vec<DetectedGene>, String> {
    let sequences =
        load_fasta_sequences(filepath).map_err(|e| format!("{filepath}: {e}"))?;
    ...
    // Rewind distance — identical to parallel_fastafile.
    ...
    if (*sw).tmrna != 0 && rewind < MAXTMRNALEN as i64 {
        rewind = MAXTMRNALEN as i64;
    }
    if (*sw).peptide != 0 && (*sw).tagthresh >= 5 && rewind < TSWEEP as i64 {
        rewind = TSWEEP as i64;
    }
    (*sw).loffset = rewind as i32;
    (*sw).roffset = rewind as i32;
    ...
    let gc_values: Vec<f64> = if parallel_enabled {
        sequences.par_iter().map(|seq| calculate_gc(&seq.seq)).collect()
    } else {
        sequences.iter().map(|seq| calculate_gc(&seq.seq)).collect()
    };
    ...
    let first_small_circular_idx: Option<usize> =
        sequences.iter().position(|seq| (seq.len() as i64) <= drewind);
    let drewind_i32 = drewind as i32;

    let effective_offsets: Vec<(i32, i32)> = sequences
        .iter()
        .enumerate()
        .map(|(idx, _)| match first_small_circular_idx {
            Some(first_idx) if idx >= first_idx => (drewind_i32, drewind_i32),
            _ => (sw_loffset, sw_roffset),
        })
        .collect();

    let (all_chunks, needs_dedup) =
        create_unified_chunks(&sequences, &gc_values, &effective_offsets, effective_threads);

    let chunk_results: Vec<(usize, Vec<SendableGene>)> = all_chunks
        .par_iter()
        .map(|chunk| {
            let sw_ptr = sw_ptr_usize as *const csw;
            let genes = process_chunk_parallel(
    ...
    let mut out = Vec::new();
    for (idx, seq_record) in sequences.iter().enumerate() {
        ...
        for g in &genes {
            let anticodon = if g.genetype == TRNA {
                decode_anticodon(g)
            } else {
                String::new()
            };
            out.push(DetectedGene {
                seqname: seq_record.id.clone(),
                start: g.start,
                stop: g.stop,
                comp: g.comp,
                genetype: g.genetype,
